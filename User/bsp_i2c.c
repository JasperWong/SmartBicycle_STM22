...
unsigned long ST_Hard_Sensors_I2C_WriteRegister(unsigned char Address, unsigned char RegisterAddr, unsigned short RegisterLen, const unsigned char *RegisterValue)
{
	 ...
	 /* Send STRAT condition */
	  ...
	  I2C_Send7bitAddress(macI2Cx, (Address<<1), I2C_Direction_Transmitter);
	  ...
	  I2C_SendData(macI2Cx, RegisterAddr);
	  ...
	  for( i=0; i<(RegisterLen); i++)
	   {
	     ...
	     I2C_SendData(macI2Cx, RegisterValue[i]);
	   }
...
unsigned long ST_Hard_Sensors_I2C_ReadRegister(unsigned char Address, unsigned char RegisterAddr, unsigned short RegisterLen, unsigned char *RegisterValue)
{
	 I2CTimeout = I2CT_LONG_TIMEOUT;
	  ...
	  I2C_Send7bitAddress(macI2Cx, (Address<<1), I2C_Direction_Receiver);
	  ...
	  while(RegisterLen)
	  {
	    if(RegisterLen == 1)
	    {
	      ...
	      I2C_GenerateSTOP(macI2Cx, ENABLE);
			...
			I2CTimeout = I2CT_LONG_TIMEOUT;
			while(!I2C_CheckEvent(macI2Cx, I2C_EVENT_MASTER_BYTE_RECEIVED))
			{
				if((I2CTimeout--) == 0) 
				{
			...
			
	    {
	      ...
	      *RegisterValue = I2C_ReceiveData(macI2Cx);
...
static  uint8_t I2C_TIMEOUT_UserCallback(uint8_t errorCode)
{
	/*重置IIC*/
  I2C_GenerateSTOP(macI2Cx, ENABLE);
  I2C_SoftwareResetCmd(macI2Cx, ENABLE);
  I2C_SoftwareResetCmd(macI2Cx, DISABLE);
